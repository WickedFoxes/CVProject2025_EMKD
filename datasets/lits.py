...
class LitsSliceDataset(Dataset):
    def __init__(self, data_path, task='tumor', series_index=None, train=True):
        """ 'lits' 데이터셋 전용 클래스 """
        super(LitsSliceDataset, self).__init__()
        ...
        self.task = task
        self.train = train
        self.dataset = 'lits' # <-- 'lits'로 고정
        
        # 'lits' 슬라이스 목록 로드
        self.slice_list_path = os.path.join(data_path, 'lits_%s_slices.npy' % task)
    ...
    def __getitem__(self, item):
        ...
        # 3. 데이터셋별 윈도잉 ('lits' 전용)
        # 'du' 모듈이 필요합니다.
        ct = du.window_standardize(ct, -60, 140) # <-- 'lits' 값 고정
